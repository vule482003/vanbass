...
export default function LocalCTA({
  ...
  const kicker = lang === "en" ? t.localCTA.kicker : (config.kicker || t.localCTA.kicker);
  const headlineTop = lang === "en" ? t.localCTA.headline_top : (config.headline_top || t.localCTA.headline_top);
  const headlineBottom = lang === "en" ? t.localCTA.headline_bottom : (config.headline_bottom || t.localCTA.headline_bottom);
  const desc = lang === "en" ? t.localCTA.desc : (config.desc || t.localCTA.desc);
  const primaryBtnText = lang === "en" ? t.localCTA.primary_btn_text : (config.primary_btn_text || t.localCTA.primary_btn_text);
  const secondaryBtnText = lang === "en" ? t.localCTA.secondary_btn_text : (config.secondary_btn_text || t.localCTA.secondary_btn_text);
    ...
    <section className="local-section reveal-on-scroll" id="contact-cta">
      <div className="container local-content">
        ...
        <h2 style={{ fontSize: "clamp(32px, 4.2vw, 56px)", fontWeight: 900, letterSpacing: "-0.03em", margin: "0 0 16px 0", color: "#ffffff" }}>
          <span data-cms-key="local_cta.headline_top" data-cms-label="Tiêu Đề Trên Showroom" data-cms-type="text">{headlineTop}</span>
          <br />
          <span style={{ color: "#71717a" }} data-cms-key="local_cta.headline_bottom" data-cms-label="Tiêu Đề Dưới Showroom" data-cms-type="text">{headlineBottom}</span>
        </h2>
